...
struct tw_maps *tw_parse_maps(const char *filename) {
  ...
  int tw_write_fd = sockets[0];
  int tw_read_fd = sockets[1];
  ...
  if (new_pid == 0) {
    ...
    tw_write_loop(tw_write_fd, &has_more_maps, sizeof(uint8_t));
  ...
  struct tw_maps *maps = (struct tw_maps *)malloc(sizeof(struct tw_maps));
  if (!maps) {
  ...
  while (1) {
    ...
    size_t path_len = 0;
    if (tw_read_loop(tw_read_fd, &path_len, sizeof(path_len)) != sizeof(path_len)) {
      PLOGE("Read path_len");
...
struct tw_mountsinfo *tw_parse_mountinfo(const char *filename) {
  ...
  struct tw_mountsinfo *mounts = (struct tw_mountsinfo *)malloc(sizeof(struct tw_mountsinfo));
  if (!mounts) {
...
#define write_func(type)                       \
  ssize_t tw_write_## type(int fd, type val) {    \
    return tw_write_loop(fd, &val, sizeof(type)); \
  }

#define read_func(type)                      \
  ssize_t tw_read_## type(int fd, type *val) {  \
    return tw_read_loop(fd, val, sizeof(type)); \
  }
...
time_t tw_mono_sec_now(void) {
  ...
  bool tw_update_mnt_ns(enum mount_namespace_state mns_state) {
    ...
    if (tw_rezygiskd_update_mns(mns_state, ns_path, sizeof(ns_path)) == false) {
...
struct tw_mem_info tw_get_mem_info(void) {
  ...
  struct tw_maps *maps = tw_parse_maps("/proc/self/maps");
  if (!maps) {
