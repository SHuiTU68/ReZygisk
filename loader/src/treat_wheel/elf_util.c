...
int tw_dl_cb(struct dl_phdr_info *info, size_t size, void *data) {
  (void) size;
...
bool tw_find_module_base(struct elf_img *img) {
  dl_iterate_phdr(tw_dl_cb, img);
...
void tw_elf_destroy(struct elf_img *img) {
  if (!img) return;
...
struct elf_img *tw_elf_create(const char *elf, void *base) {
...
bool tw_load_symtabs(struct elf_img *img) {
...
ElfW(Addr) tw_getSymbOffset(struct elf_img *img, const char *name, unsigned char *sym_type) {
  ElfW(Addr) offset = 0;
...
ElfW(Addr) tw_getSymbAddress(struct elf_img *img, const char *name) {
  unsigned char sym_type = 0;
  ElfW(Addr) offset = tw_getSymbOffset(img, name, &sym_type);
...
ElfW(Addr) tw_getSymbAddressByPrefix(struct elf_img *img, const char *prefix) {
  unsigned char sym_type = 0;
...
void *tw_getSymbValueByPrefix(struct elf_img *img, const char *prefix) {
  ElfW(Addr) address = tw_getSymbAddressByPrefix(img, prefix);
